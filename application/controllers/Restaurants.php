...
class Restaurants extends CI_Controller {
    ...
    function index() {
        if ($this->session->userdata('restaurants_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(base_url(), 'refresh');
        }

        $data['page_name'] = 'dashboard';
        $data['page_title'] = get_phrase('restaurants_dashboard');
...
function medical_health_record($task = "", $id = "") {
            ...
            redirect('restaurants/medical_health_record');
 ...
 function manage_staff($task = "", $user_id = "") {
    if ($this->session->userdata('restaurants_login') != 1) {
    	...
    	if ($task == "update") {
    		...
    		redirect('restaurants/manage_staff');
    	}
...
function invoice_add($task = "") {
    		...
    		if($data == FALSE){
    			...
    			for($i=0; $i<sizeof($this->input->post('quantity'));$i++){
    				...
    				if ($query !== FALSE)
    				{
    					...
    					redirect('restaurants/invoice_manage');
    				}
    				else
    				{
    ...
    function receptionist($task = "", $receptionist_id = "") {
    	...
    	if ($task == "create") {
    		...
    		redirect(base_url() . 'index.php?restaurants/receptionist');
    	}
...
function medicine_category($task = "", $medicine_category_id = "")
    {
    	...
    	if ($task == "create")
    	{
    		...
    		redirect(base_url() .  'index.php?restaurants/medicine_category');
    	}
...
function medicine_sub_category($task = "", $medicine_sub_category_id = "")
    {
    	...
    	if ($task == "create")
    	{
    		...
    		redirect(base_url() .  'index.php?restaurants/medicine_sub_category');
    	}
    ...
    function stock($task = "", $medicine_id = "")
    {
    	...
    	if ($task == "delete")
    	{
    		...
    		redirect(base_url() .'index.php?restaurants/stock');
    	}
...
function patient($task = "", $patient_id = "") {
    	...
    	$tbl_name = 'patient';
	    	if ($task == "create") {
	    		...
	    		$this->session->set_flashdata('message', get_phrase('new_patient_created_sucessfully'));
	    		 redirect(base_url() . 'index.php?restaurants/patient');
	    	}
    ...
    function bed_allotment($task = "", $bed_allotment_id = "") {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'index.php?restaurants/bed_allotment');
        }
    ...
    function report($task = "", $report_id = "") {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'index.php?restaurants/report');
        }
    ...
    function profile($task = "") {
		...
		$tbl_name = "users";
        $restaurants_id = $this->session->userdata('login_user_id');
        if ($task == "update") {
        		if(isset($_POST['name'])){
        			$data['name'] = $this->input->post('name');
        			$this->crud_model->update($tbl_name,$data,$restaurants_id);
        		}
                ...
                redirect(base_url() . 'index.php?restaurants/profile');
        }

        if ($task == "change_password") {
            $password = $this->db->get_where('users', array('id' => $restaurants_id))->row()->password;
            ...
            if ($password == $old_password && $new_password == $confirm_new_password) {
                ...
                $this->db->where('id', $restaurants_id);
...
function appointment($task = "", $appointment_id = "") {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'index.php?restaurants/appointment');
        }
 ...
 function appointment_requested($task = "", $appointment_id = "") {
        ...
        if ($task == "approve") {
            ...
            redirect(base_url() . 'index.php?restaurants/appointment_requested');
        }
    ...
    function prescription($task = "", $prescription_id = "", $menu_check = '', $patient_id = '') {
        ...
        if ($task == "create") {
            ...
            redirect(base_url() . 'index.php?restaurants/prescription');
        }
        ...
        if ($task == "update") {
            ...
            if ($menu_check == 'from_prescription')
                ...
                redirect(base_url() . 'index.php?restaurants/medication_history/' . $patient_id);
        }
	...
	function message($param1 = 'message_home', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'send_new') {
            ...
            redirect(base_url() . 'index.php?restaurants/message/message_read/' . $message_thread_code, 'refresh');
        }

        if ($param1 == 'send_reply') {
            $this->crud_model->send_reply_message($param2);  //$param2 = message_thread_code
            $this->session->set_flashdata('message', get_phrase('message_sent!'));
            redirect(base_url() . 'index.php?restaurants/message/message_read/' . $param2, 'refresh');
        }
    			...
    			if($this->input->post('selected_mail_option') == "All Staff"){
    				...
    				if($query->num_rows() != 0)
    				{
    					...
    					if($email == TRUE){
    						...
    						redirect('/restaurants/staff_message', 'refresh');
    					}
    					return $query->result_array();
    				}
    				else
    ...
    function blood_donor($task = "", $blood_donor_id = "") {
    	...
    	if ($task == "create") {
    		...
    		redirect(base_url() . 'index.php?restaurants/blood_donor');
    	}
