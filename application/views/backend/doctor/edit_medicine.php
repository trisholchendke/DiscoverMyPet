...
$medicine_sub_category_info = $this->db->get_where('medicine_sub_category',array("doctor_id" =>  $this->session->userdata('login_user_id') ))->result_array();
...
<button type="button" class="close pull-right close_btn" data-dismiss="modal" aria-hidden="true">×</button>
                ...
                <div class="panel-body">
                    ...
                    <form role="form" class="form-horizontal form-groups-bordered" action="<?php echo base_url(); ?>index.php?doctor/stock/update/<?php echo $edit_data[0]['medicine_id']; ?>" method="post" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-6">
                            <select id="medicine_category_id" onchange="get_medicine_subcategory(this)" name="medicine_category_id" class="form-control">
                                <?php 
...
if($query->num_rows() != 0)
    		{ $medicine_category_info = $query->result_array()?>
    		 ...
    		 <?php foreach ($medicine_category_info as $row) { ?>
                                    <option value="<?php echo $row['medicine_category_id']; ?>" <?php if($edit_data[0]['medicine_category_id'] == $row['medicine_category_id']) echo "selected"?> ><?php echo $row['name']; ?></option>
                                <?php } ?>
    		<?php
    		...
    		?>
                     ...
                     <div class="form-group" id="edit_medicine">
                        ...
                        <div class="col-sm-4">
                            <select  name="medicine_sub_category_id" class="form-control">
                            	<?php foreach ($medicine_sub_category_info as $row) { ?>
                                    <option value="<?php echo $row['id']; ?>" <?php if ($edit_data[0]['medicine_sub_category_id'] == $row['id']) echo 'selected'; ?>><?php echo $row['name']; ?></option>
                                <?php } ?>
                            </select>
                        </div>
                    </div>
