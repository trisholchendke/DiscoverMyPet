<?php 
	$doctor_info = $this->db->get('doctor',array('doctor_id',$this->session->userdata('doctor_id')))->result_array(); 
	$this->db->select('*');
    	$this->db->from('users');
    	$this->db->where('doctor_id',$this->session->userdata('doctor_id'));
    	$this->db->where('role','Sub Doctor');
    	$query = $this->db->get();
    	$doctor_info1 = $query->result_array();
?>
<table class="table table-bordered table-striped datatable" id="table-2">
    ...
    <tbody>
        ...
        <?php } ?>
        <?php foreach ($doctor_info1 as $row) { ?>   
            <tr>
                <td>
                    <img src="<?php echo $this->crud_model->get_image_url('doctor' , $row['doctor_id']);?>" 
                         class="img-circle" width="40px" height="40px">
